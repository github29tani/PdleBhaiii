-- Add debugging to notification function
...
BEGIN
  ...
  RAISE NOTICE 'New notification: id=%, target=%', NEW.id, NEW.target;
  ...
  IF NEW.target IS NULL OR NEW.target = '{}'::jsonb OR NEW.target = '{"classes":[],"subjects":[],"boards":[],"languages":[]}'::jsonb THEN
    RAISE NOTICE 'Sending to everyone because target is null/empty';
    INSERT INTO notification_recipients (notification_id, user_id)
    SELECT NEW.id, id FROM profiles;
    RAISE NOTICE 'Inserted % recipients', (SELECT COUNT(*) FROM notification_recipients WHERE notification_id = NEW.id);
    RETURN NEW;
  END IF;
  ...
  -- Log extracted arrays
  RAISE NOTICE 'Extracted arrays: classes=%, subjects=%, boards=%, languages=%', 
    target_classes, target_subjects, target_boards, target_languages;
  ...
  -- Log send to all status  
  RAISE NOTICE 'is_send_to_all: %', is_send_to_all;

  -- Insert into notification_recipients
  IF is_send_to_all THEN
    -- Send to everyone if no specific targets
    RAISE NOTICE 'Sending to everyone because all arrays are empty';
    INSERT INTO notification_recipients (notification_id, user_id)
    SELECT NEW.id, id FROM profiles;
    RAISE NOTICE 'Inserted % recipients', (SELECT COUNT(*) FROM notification_recipients WHERE notification_id = NEW.id);
  ELSE
    ...
    RAISE NOTICE 'Inserted % recipients for targeted notification', 
      (SELECT COUNT(*) FROM notification_recipients WHERE notification_id = NEW.id);
  END IF;
